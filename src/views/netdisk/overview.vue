...
<script lang="ts" setup>
  ...
  const initData = async () => {
    try {
      ...
      flowChartOption.value.series![0].data = data.flowTrend.datas.map((e) => (e / 1024 / 1024).toFixed())
      // @ts-ignore
      spaceChartOption.value.xAxis!.data = data.sizeTrend.times
      spaceChartOption.value.series![0].data = data.sizeTrend.datas.map((e) => (e / 1024 / 1024).toFixed())
    } finally {
      hideLoading()
    }
  }
