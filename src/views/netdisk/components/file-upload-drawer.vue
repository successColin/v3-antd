<template>
  <div class="file-upload-drawer-container">
    <Drawer :title="title" :width="400" :visible="visible" :mask-closable="false" @close="handleClose">
...
<script lang="ts" setup>
  ...
  const title = computed(() => {
    return `上传文件到${isEmpty(path.value) ? '根' : path.value.substring(0, path.value.length - 1)}目录`
  })
  ...
  const uploadFile: UploadProps['customRequest'] = (param) => {
    ...
    const observable = qiniu.upload(file as File, `${path.value}${(file as File).name}`, token.value)
  ...
  const handleUploadError = (err, file: File) => {
    ...
    notification.error({
      message: '上传进度提醒',
      description: `上传${file?.name}文件失败！错误信息：${err.code === 614 ? '上传文件已存在' : err.message}`,
      duration: 0
    })
  }
