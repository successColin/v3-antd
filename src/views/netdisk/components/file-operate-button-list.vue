    ...
    <Dropdown size="small">
      ...
      <template #overlay>
        <Menu @click="handleMoreOpCommand">
          <Menu.Item key="copy" :disabled="!$auth('netdisk:manage:copy')"> <CopyOutlined /> 复制所选 </Menu.Item>
          <Menu.Item key="cut" :disabled="!$auth('netdisk:manage:cut')">
            <ScissorOutlined :rotate="-90" /> 剪切所选
          </Menu.Item>
          <Menu.Item key="delete" :disabled="!$auth('netdisk:manage:delete')"> <DeleteOutlined /> 删除所选 </Menu.Item>
          <Menu.Item key="cancel" divider :disabled="!copyMode && !cutMode"> <CloseOutlined /> 取消粘贴 </Menu.Item>
        </Menu>
      </template>
    </Dropdown>
