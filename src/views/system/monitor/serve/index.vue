    ...
    <Card class="stat-card" title="CPU">
      <Descriptions :column="1" :label-style="{ width: '50%' }" :content-style="{ width: '50%' }">
        ...
        <Descriptions.Item label="负载">
          <Progress
            :percent="formarPercentage(cpu.rawCurrentLoad, cpu.rawCurrentLoadIdle + cpu.rawCurrentLoad)"
            :stroke-color="customProgressColor"
          />
        </Descriptions.Item>
        <Descriptions.Item v-for="(item, index) in cpu.coresLoad" :key="index" :label="`核心${index + 1} 负载`">
          <Progress
    ...
    <Card class="stat-card" title="磁盘">
      <div class="disk-info">
        ...
        <div class="disk-info--item">
          <Progress type="dashboard" :percent="parseDiskPercentage" :width="100" :stroke-color="customProgressColor" />
        </div>
      </div>
    </Card>
    <!-- memoty -->
