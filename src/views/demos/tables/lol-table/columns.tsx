...
const AvatarRender = defineComponent({
  ...
  setup(props) {
    ...
    return () => (
      <>
        <Image src={props.record?.avatar} preview={{ visible: false }} onClick={() => (visible.value = true)} />
        <div hidden>
          <Image.PreviewGroup preview={{ visible: visible.value, onVisibleChange: (vis) => (visible.value = vis) }}>
